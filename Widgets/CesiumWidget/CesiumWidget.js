...
function startRenderLoop(widget) {
  ...
  // console.log(widget.canvas.clientWidth)
  function render(frameTime, frame) {
  ...
  // widget._session.requestReferenceSpace('local').then((refSpace) => {
  //   xrReferenceSpace = refSpace;
  // })
  // .then(setupWebGLLayer(context._gl, widget._session)) // Create a compatible XRWebGLLayer
  // .then(() => {
    // Start the render loop
    widget._session.requestAnimationFrame(render);
  // });
}
 ...
 * });
 */
function CesiumWidget(container, canvas, gl, element, options) {
 ...
 */
CesiumWidget.prototype.render = function (session) {
  if (this._canRender) {
    this._scene.initializeFrame();
    var currentTime = this._clock.tick();
    this._scene.render(currentTime, session);
  } else {
    this._clock.tick();
  }
};
