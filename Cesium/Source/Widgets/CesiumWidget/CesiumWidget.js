...
function getDefaultSkyBoxUrl(suffix) {
  return buildModuleUrl(
    "Assets/Textures/SkyBox2/space_" + suffix + ".jpg"
  );
}
...
function startRenderLoop(widget) {
  ...
  widget._engine.runRenderLoop(render);

  // widget._babylonScene.registerBeforeRender(render);
  // requestAnimationFrame(render);
}
...
function CesiumWidget(container, options) {
  ...
  try {
    ...
    if (!defined(skyBox)) {
      skyBox = new SkyBox({
        babylonScene: this._babylonScene,
        sources: {
          positiveX: getDefaultSkyBoxUrl("left"),
          negativeX: getDefaultSkyBoxUrl("up"),
          positiveY: getDefaultSkyBoxUrl("front"),
          negativeY: getDefaultSkyBoxUrl("right"),
          positiveZ: getDefaultSkyBoxUrl("down"),
          negativeZ: getDefaultSkyBoxUrl("back"),
        },
      });
    }
    if (skyBox !== false) {
...
let tmpCameraUp = new BABYLON.Vector3(0, 0, 0);
/**
...
async function createBabylonScene(widget) {
  //创建babylon scene
  widget._engine = new BABYLON.Engine(widget.canvas, false);
  widget._babylonScene = new BABYLON.Scene(widget._engine);
  //使用右手坐标系
  widget._babylonScene.useRightHandedSystem = true;
  widget._babylonScene.useGeometryUniqueIdsMap = true;
  widget._babylonScene.useMaterialMeshMap = true;
  widget._babylonScene.useClonedMeshMap = true;

  widget.activeCamera = new BABYLON.ArcRotateCamera(
    "camera",
    0,
    0,
    10000000000,
  ...
  var lightdown = new BABYLON.HemisphericLight(
    "light2",
    new BABYLON.Vector3(0, -1, 0),
    widget._babylonScene
  );
  ...
  const pipeline = new BABYLON.DefaultRenderingPipeline(
    ...
    widget._babylonScene, // The scene instance
    [widget.activeCamera] // The list of cameras to be attached to
  );
  // toLinearSpace
  pipeline.samples = 4;
