...
ShaderProgram.prototype.toBabylonShader = function (options) {
  let name = options.name;
  let mesh = options.mesh;
  let babylonScene = options.babylonScene;
  let debug = defaultValue(options.debug, false);
  let uniformMap = options.uniformMap;
  let attributes = defaultValue(options.attributes, ['position']);

  let vs = this._vertexShaderText;
  let fs = this._fragmentShaderText;
  ...
  var czmMatchesVS = vs.match(/\bczm_[a-zA-Z0-9_]*/g);
  var czmMatchesFS = fs.match(/\bczm_[a-zA-Z0-9_]*/g);
  let czmMatches = czmMatchesVS.concat(czmMatchesFS);
  let updateUniforms = [];  //需要一直更新的value
  if (defined(czmMatches) && czmMatches !== null) {
    ...
    czmMatches.forEach(function (element) {
      if(AutomaticUniforms.hasOwnProperty(element)) {
        updateUniforms.push(element);
      }
    });
  }

  let difinesVS = this._vertexShaderSource.defines;
  let difinesFS = this._fragmentShaderSource.defines;
  let defines = difinesVS.concat(difinesFS);
  defines = defines.filter(function (elem, pos) {
    return defines.indexOf(elem) === pos;
  });
  let babydefine = ["#define HDR"];
  for (let i = 0; i < defines.length; i++) {
    if(defines[i] !== "") {
      babydefine.push("#define "+ defines[i]);
    }
  }

  let material = new BABYLON.ShaderMaterial(name, babylonScene, {
    vertexSource:vs,
    fragmentSource: fs
  }, {
  ...
  if(defined(uniformMap)){
    for (const key in uniformMap) {
      if(typeof uniformMap[key] === 'function') {
        let value = uniformMap[key]();
        if(value instanceof Cartesian3) {
          ...
          material.setMatrix(key, value.toBabylonMatrix());
        } else if(value instanceof Color) {
          material.setColor4(key, new BABYLON.Color4(value.red, value.green, value.blue, value.alpha));
        } else if(Array.isArray(value)){
          let type= value[0];
          if(type instanceof Cartesian4) {
            let array = [];
            for (let j = 0; j < value.length; j++) {
              array.push(value[j].x);
              array.push(value[j].y);
              array.push(value[j].z);
              array.push(value[j].w);
            }
            material.setArray4(key, array);
          } else if(type instanceof Cartesian3) {
            let array = [];
            for (let j = 0; j < value.length; j++) {
              array.push(value[j].x);
              array.push(value[j].y);
            ...
            material.setArray3(key, array);
          } else if(type instanceof BABYLON.Texture) {
            material.setTextureArray(key, value);
          } else {
            material.setFloats(key, value);
          }
