 ...
 * @param mesh
 */
VertexArray.prototype.toBabylonBuffer = function (mesh) {
  let engine = mesh.getEngine();
  let attrbutes = this._attributes;

  for (let i = 0; i < attrbutes.length; i++) {
    let attribute = attrbutes[i];
    let name = attribute.name;
    let componentDatatype = attribute.componentDatatype;
    let normalize = attribute.normalize;
    let componentsPerAttribute = attribute.componentsPerAttribute;
    let typedArray = attribute.vertexBuffer._typedArray;
    let strideInBytes = attribute.strideInBytes;
    let offsetInBytes = attribute.offsetInBytes;
    // let sizeInBytes = attribute.vertexBuffer.sizeInBytes;
    const buffer = new BABYLON.Buffer(engine, typedArray, false);
    var vertexBuffer = new BABYLON.VertexBuffer(engine, buffer, name, false, false, strideInBytes, false, offsetInBytes, componentsPerAttribute, componentDatatype, normalize, true);
    mesh.setVerticesBuffer(vertexBuffer);
  }
  mesh.setIndices(this._indexBuffer._typedArray);
}

VertexArray.prototype.toBabylonBuffer2 = function (mesh) {
  let engine = mesh.getEngine();
  let attrbutes = this._attributes;
  ...
  for (let i = 0; i < attrbutes.length; i++) {
    ...
    // let offsetInBytes = attribute.offsetInBytes;
    // let strideInBytes = attribute.strideInBytes;
    bufferSize+=componentsPerAttribute;
  }
  let typedArray = attrbutes[0].vertexBuffer._typedArray;
  let buffer = new BABYLON.Buffer(engine, typedArray, false, bufferSize);
  ...
  for (let i = 0; i < attrbutes.length; i++) {
    ...
    let componentsPerAttribute = attribute.componentsPerAttribute;
    let offsetInBytes = attribute.offsetInBytes;
    let offset = offsetInBytes / typedArray.BYTES_PER_ELEMENT
    mesh.setVerticesBuffer(buffer.createVertexBuffer(name, offset, componentsPerAttribute));
  }
  mesh.setIndices(this._indexBuffer._typedArray);
}
