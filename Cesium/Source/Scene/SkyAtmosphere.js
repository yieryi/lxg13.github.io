...
SkyAtmosphere.prototype.update = function (frameState, globe) {
  ...
  if (!defined(command.vertexArray)) {
    ...
    let engine = frameState.context.engine;
    let scene = engine.scenes[0]
    this._skyAtomosphereMesh = new BABYLON.Mesh('skyAtmosphere', scene);
    // this._skyAtomosphereMesh.scaling = new BABYLON.Vector3(1,-1,1);
    this._skyAtomosphereMesh.renderingGroupId = 1;
    command.vertexArray.toBabylonBuffer(this._skyAtomosphereMesh);

    // this._skyAtomosphereMesh.registerBeforeRender(function() {
    //
    //   // engine._depthCullingState.cull = true;
    //   // engine._depthCullingState.cullFace = engine._gl.FRONT;
    //   // engine.setAlphaEquation(BABYLON.Constants.ALPHA_EQUATION_ADD);
    //   // engine.setAlphaMode(BABYLON.Constants.ALPHA_SCREENMODE);
    //   engine.setDepthWrite(false);
    // });

    // this._skyAtomosphereMesh.registerAfterRender(function() {
    //   engine._depthCullingState.reset();
    //   engine._alphaState.reset();
    //
    // });

    // const myMaterial = new BABYLON.StandardMaterial("myMaterial", scene);
    // this._skyAtomosphereMesh.material = myMaterial;
    this._skyAtomosphereMesh.setPreTransformMatrix(this._modelMatrix.toBabylonMatrix());
  }

  // //更新modelMatrix
  let us = context.uniformState;
  us.model = this._modelMatrix;
  ...
  if (flags !== this._flags) {
    ...
    this._spSkyFromSpaceMaterial = this._spSkyFromSpace.toBabylonShader({
      name: 'skyAtmosphereSpace',
      mesh: this._skyAtomosphereMesh,
      babylonScene:  this.babylonScene,
      uniformMap: command.uniformMap
      // debug: true
    });

    this._spSkyFromSpaceMaterial.transparencyMode = BABYLON.Material.MATERIAL_ALPHABLEND;
    this._spSkyFromSpaceMaterial.alpha = 0.5;


    this._spSkyFromAtmosphereMaterial = this._spSkyFromAtmosphere.toBabylonShader({
      name: 'skyAtmosphereAtmosphere',
      mesh: this._skyAtomosphereMesh,
      babylonScene:  this.babylonScene,
      uniformMap: command.uniformMap
      // debug: true
    ...
    this._spSkyFromAtmosphereMaterial.transparencyMode = BABYLON.Material.MATERIAL_ALPHABLEND;
    this._spSkyFromAtmosphereMaterial.alpha = 0.5;
  ...
  if (cameraHeight > this._radiiAndDynamicAtmosphereColor.x) {
    // Camera in space
    this._skyAtomosphereMesh.material = this._spSkyFromSpaceMaterial;
    // command.shaderProgram = this._spSkyFromSpace;
  } else {
    // Camera in atmosphere
    this._skyAtomosphereMesh.material = this._spSkyFromAtmosphereMaterial;
    // command.shaderProgram = this._spSkyFromAtmosphere;
  }

  let material = this._skyAtomosphereMesh.material;
  ...
  for (let i = 0; i < updateUniforms.length; i++) {
    ...
    if(uniform) {
      ...
      } else if(type === WebGLConstants.FLOAT_VEC3) {
        material.setVector3(name, uniform.getValue(us));
      }
    } else {
      material.setVector3(name, command.uniformMap[name]());
    }
