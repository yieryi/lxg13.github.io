...
import RTTCommand from "../Renderer/RTTCommand.js";
...
ImageryLayer.prototype._createTextureWebGL = function (context, imagery) {
  ...
  //   sampler: sampler,
  // });
  let scene = context.babyScene;
  let name = imagery.x+'-'+imagery.y+'-'+imagery.level;
  let texture = new BABYLON.Texture(name, scene, true,
    false, BABYLON.Texture.LINEAR_LINEAR_MIPNEAREST,
    null, null, image, true,
    this._imageryProvider.hasAlphaChannel?BABYLON.Engine.TEXTUREFORMAT_RGBA: BABYLON.Engine.TEXTUREFORMAT_RGB);
...
ImageryLayer.prototype._reprojectTexture = function (
  ...
  if (
    ...
    rectangle.width / texture.getSize().width > 1e-5
  ) {
    var that = this;
    ...
    // });
    //     imagery.texture = texture;
    //     that._finalizeReprojectTexture(context, texture);
    //     imagery.state = ImageryState.READY;
    //     imagery.releaseReference();
    // this._reprojectComputeCommands.push(computeCommand);
    let scene = context.engine.scenes[0];
    var computeCommand = new RTTCommand({
      persists: true,
      owner: this,
      name: 'imagery',
      scene: scene,
      ...
      // This allows different ImageryLayers to share the same vao and buffers.
      // preExecute: function (command) {
      //     test(command, context, texture, imagery.rectangle)
      //     // reprojectToGeographic(command, context, texture, imagery.rectangle);
      // },
      postExecute: function (outputTexture) {
        // imagery.texture = new BabylonTexture('templates/waterNormals.jpg', scene);
        imagery.texture = outputTexture;
        that._finalizeReprojectTexture(context, outputTexture);
        imagery.state = ImageryState.READY;
        imagery.releaseReference();
    ...
    test(computeCommand, context, texture, imagery.rectangle);
...
function test(command, context, texture, rectangle) {
  let engine = context.engine;
  var mesh = context.cache.imageryLayer_reproject;
  if (!defined(mesh)) {
    let vertexShader = `
    ...
    mesh = new BABYLON.Mesh("quad");
    mesh.isPickable = false;
    let buffer = new BABYLON.Buffer(engine, positions, false, 2);
    // let buffer2 = new BABYLON.Buffer(engine, webMercatorT, false, 1);
    mesh.setVerticesBuffer(buffer.createVertexBuffer('position', 0, 2));
    // mesh.setVerticesBuffer(buffer2.createVertexBuffer('webMercatorT', 0, 1));
    ...
    let material = new BABYLON.ShaderMaterial(
      ...
      {
        attributes: [ 'position', 'webMercatorT'],
      }
    )
    mesh.material = material;

    command.outputTexture.renderList.push(mesh);
  }

  var width = texture.getSize().width;
  var height = texture.getSize().height;
  ...
  material.setVector2("u_textureDimensions", new BABYLON.Vector2(width, height));
  // var viewport = new BoundingRectangle();
  viewport.width = width;
  viewport.height = height;

  context.uniformState.viewport = viewport;
  context.uniformState._viewportDirty = true;
  material.setMatrix('czm_viewportOrthographic', context.uniformState.viewportOrthographic.toBabylonMatrix())

  // command.outputTexture.renderList.splice(0);
